...
class TwistedTowerViewSet(viewsets.ModelViewSet):
    ...
    def create_twisted_tower(self, request):
        ...
        pre_vm.delete()
        # モデルを作成
        item = TwistedTower(title=title, base_curve=base_curve, center_point=center_point, twisted_tower=twisted_tower,
                            angle=angle, height=height, created_by=user)
        item.save()

        # シリアライズ
        serializer = TwistedTowerSerializer(
            data={"title": title, "base_curve": base_curve, "center_point": center_point,
                  "twisted_tower": twisted_tower,
